...
fetch('assets/js/mock-data.json')
    .then(response => response.json())
    .then(data => {
        //shuffle the array inside the json,
        const container = document.querySelector('.tweets-container');

        // create a new array using the ...data, this will not affect the original array from the json file
        const shuffledData = [...data].sort(() => 0.5 - Math.random());

        // display the randomized new array
        shuffledData.forEach(post => {
            ...
            article.innerHTML = `
                <div class="tweet-container">
                    <figure class="post-tweet-pf">
                            <img src="${post.profile_image}" class="post-tweet-image">
                    </figure>
                    <div class="account-tweet">
                        <div class="account-top">
                            <div class="account-name">
                                <h2>${post.author}</h2>
                                <h4>@${post.username}</h4>
                                <h4><span class="dot">•</span> ${post.timestamp}</h4>
                            </div>
